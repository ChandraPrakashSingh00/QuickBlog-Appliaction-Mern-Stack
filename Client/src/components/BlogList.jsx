  ...
  return (
    <div>
        ...
        {blogCategories.map((item) => (
          <div key={item} className='relative'>
            <button
              ...
              className={`cursor-pointer px-4 pt-0.5 rounded-full transition duration-300 ${
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-8 mb-24 mx-8 sm:mx-16 xl:mx-40'>
        {filteredBlogs().filter((blog) => menu === "All"? true :blog.category === menu).map((blog) => <BlogCard key={filteredBlogs} blog={blog}/>)}
      </div>
    </div>
  );
};
